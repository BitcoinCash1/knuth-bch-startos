...
- **`tagAndRelease.yml`** — on push to `master` (or when dispatched after an auto-bump), builds kth from source, tags `v<version>` and publishes a GitHub release with the built `.s9pk` files.
- **`syncNext.yml`** — mirrors `master` into `next`.
- **`check-upstream.yml`** — daily check for a new upstream Knuth release; commits the bump to `master` and dispatches `tagAndRelease.yml`.
