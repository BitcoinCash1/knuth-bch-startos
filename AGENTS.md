...
- **kth is compiled from source in the `Dockerfile`, with `rpc=True`.** The official `ghcr.io/k-nuth/kth` image is built without the JSON-RPC server, so it cannot be used as a base yet. The build takes a long time on a hosted runner; that is expected. Upstream images come first: `check-upstream.yml` runs `scripts/use-upstream-image.sh` daily, which switches the `Dockerfile` to `scripts/Dockerfile.upstream` (a wrapper around the official image) the moment the image for the pinned version ships JSON-RPC (k-nuth/docker-images#7).
